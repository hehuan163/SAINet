...
from dataset.deep_globe import RGB_mapping_to_class#增加
import cv2
import numpy as np
import torch
...
# def global2patch(images, entropy_maps, p_size, entropy_threshold):#修改后
#     '''
#     image/label => patches
#     entropy_maps: entropy maps with shape (batch_size, height, width)
#     p_size: patch size
#     entropy_threshold: threshold of entropy to select a patch
#     return: list of PIL patch images; coordinates: images->patches; ratios: (h, w)
#     '''
#     patches = []; coordinates = []; templates = []; sizes = []; ratios = [(0, 0)] * len(images); patch_ones = np.ones(p_size)
#     b, H, W = entropy_maps.size()
...
#         high_uncertainty_patches = []
#         high_uncertainty_coordinates = []
...
#                 else: left = size[1] - p_size[1]
#                 template[top:top+p_size[0], left:left+p_size[1]] += patch_ones
#                 patch = transforms.functional.crop(images[i], top, left, p_size[0], p_size[1])
#                 # measure the uncertainty of the patch
#                 top_scaled, left_scaled = int(np.round(top * H / size[0])), int(np.round(left * W / size[1]))
#                 h_scaled, w_scaled = int(np.round(p_size[0] * H / size[0])), int(np.round(p_size[1] * W / size[1]))
#                 entropy_patch = entropy_maps[i, top_scaled:top_scaled+h_scaled, left_scaled:left_scaled+w_scaled]
#                 entropy_mean = entropy_patch.mean()
#                 if entropy_mean > entropy_threshold:
#                     high_uncertainty_patches.append(patch)
#                     high_uncertainty_coordinates.append((1.0 * top / size[0], 1.0 * left / size[1]))

#         patches.append(high_uncertainty_patches)
#         coordinates.append(high_uncertainty_coordinates)
#         templates.append(Variable(torch.Tensor(template).expand(1, 1, -1, -1)).cuda())#torch.Tensor(template).expand(1, 1, -1, -1).cuda()
#     return patches, coordinates, templates, sizes, ratios



# def global2patch(images, entropy_maps, p_size, entropy_threshold):
#     '''
#     image/label => patches
#     entropy_maps: entropy maps with shape (batch_size, height, width)
#     p_size: patch size
...
#     while all(not patch for patch in patches): # Keep reducing entropy_threshold until patches is not empty
#         patches = []; coordinates = []; templates = []; sizes = []; ratios = [(0, 0)] * len(images); patch_ones = np.ones(p_size)
#         b, H, W = entropy_maps.size()
...
def global2patch(images, entropy_maps, p_size, entropy_threshold):
    ...
    entropy_maps: entropy maps with shape (batch_size, height, width)
    p_size: patch size
    entropy_threshold: initial threshold of entropy to select a patch
    return: list of PIL patch images; coordinates: images->patches; ratios: (h, w)
    '''
    patches = []; coordinates = []; templates = []; sizes = []; ratios = [(0, 0)] * len(images); patch_ones = np.ones(p_size)
    b, H, W = entropy_maps.size()

    while True:  # start a loop that will only break when all images have at least one patch
        for i in range(b):
            w, h = images[i].size
            size = (h, w)
            sizes.append(size)
        ...
        if all(patches):  # if all sublists in patches are non-empty, break the loop
            break

        # if any sublist in patches is empty, clear lists and reduce the threshold for the next iteration
        patches = []
        coordinates = []
        templates = []
        sizes = []
...
class Trainer(object):
    ...
    def train(self, sample, model, global_fixed):
        ...
        feature_global, _ = model.forward(images_glb, None, None, None)#修改（增加）
        e_map = model.module._calculate_entropy(feature_global)#修改（增加）

        
        # e_map_unsqueezed = e_map.unsqueeze(1)
        # e_map_repeated = e_map_unsqueezed.repeat(1, 7, 1, 1)
        # print(feature_global.shape)
        # print(e_map.shape)
#         e_map_exp = e_map.unsqueeze(1).expand(-1, 7, -1, -1)
#         # print(e_map_exp.shape)
#         feature_global_clone = feature_global.clone()
#         e_map_exp_clone = e_map_exp.clone()
        

        # print(e_map)
        entropy_threshold = 0.08* torch.log2(torch.tensor(7.0))#修改（增加）
        ...
        labels_npy*=-255#
        labels_glb*=-255#
        array = labels_glb.cpu().numpy()#
        
        labels2 = np.zeros_like(array[: ,:, :, 0], dtype=np.int64)#
        labels3 = np.zeros_like(labels_npy[: ,:, :, 0])#
        
        
        for i in range(array.shape[0]):#
           labels2[i] = RGB_mapping_to_class(array[i])#
        
        for i in range(labels_npy.shape[0]):#
           labels3[i] = RGB_mapping_to_class(labels_npy[i])#
        
        labels_npy = labels3#
        labels_glb = torch.tensor(labels2)#
        labels_glb = labels_glb.to('cuda')#
        ...
        if self.mode == 2 or self.mode == 3:
            ...
            patches, coordinates, templates, sizes, ratios = global2patch(images, e_map, self.size_p, entropy_threshold)#修改
            
            # print(len(patches))#
            
            label_patches, _, _, _, _ = global2patch(labels, e_map, self.size_p, entropy_threshold)#修改
        ...
        if self.mode == 1:
            ...
            # print(f"Shape of outputs_global: {outputs_global.shape}")
            # print(f"Shape of labels_glb: {labels_glb.shape}")
        ...
        if self.mode == 2:
            ...
            for i in range(len(images)):
                ...
                while j < len(coordinates[i]):
                    ...
                    # print(f"344{type(label_patches)}")
                    # print(patches_var)
                    ...
                    # print(f"345{label_patches_var.shape}")
                    

                    label_patches_var*=-255#
                    array1 = label_patches_var.cpu().numpy()#
                    labels4 = np.zeros_like(array1[: ,:, :, 0])#, dtype=np.int64  
                    for k in range(array1.shape[0]):#
                       labels4[k] = RGB_mapping_to_class(array1[k])#               
                    label_patches_var = torch.tensor(labels4)#
                    label_patches_var = label_patches_var.to('cuda')#
                    
                    # e_map_expanded = e_map.unsqueeze(1).repeat(1, 7, 1, 1)#修改（增加）
                    # print(e_map_expanded.shape)
                    # print(feature_global.shape)
                    # entropy_loss = self.criterion(feature_global, e_map_expanded)#
                    ...
                    # loss = self.criterion(output_patches, label_patches_var) + self.criterion(output_ensembles, label_patches_var) + self.lamb_fmreg * fmreg_l2 + 0.01*mse#
        ...
        if self.mode == 3:
            ...
            for i in range(len(images)):
                ...
                while j < len(coordinates[i]):
                    ...
                    label_patches_var*=-255#
                    array10 = label_patches_var.cpu().numpy()#
                    labels40 = np.zeros_like(array10[: ,:, :, 0])#, dtype=np.int64  
                    for k in range(array10.shape[0]):#
                       labels40[k] = RGB_mapping_to_class(array10[k])#               
                    label_patches_var = torch.tensor(labels40)#
                    label_patches_var = label_patches_var.to('cuda')#
            ...
            for flip in self.flip_range:
                ...
                for angle in self.rotate_range:
                    ...
                    if self.mode == 2 or self.mode == 3:
                        ...
                        patches, coordinates, templates, sizes, ratios = global2patch(images, e_map, self.size_p, entropy_threshold)#修改后
